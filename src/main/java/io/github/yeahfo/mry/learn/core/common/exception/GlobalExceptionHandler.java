...
import static io.github.yeahfo.mry.learn.core.common.exception.MryException.*;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity< ErrorRepresentation > handleAccessDinedException( HttpServletRequest request ) {
        return createErrorResponse( accessDeniedException( ), request.getRequestURI( ) );
    }

    @ResponseBody
    @ExceptionHandler( { AuthenticationException.class } )
    public ResponseEntity< ErrorRepresentation > handleAuthenticationFailedException( HttpServletRequest request ) {
    ...
    public ResponseEntity< ErrorRepresentation > handleServletRequestBindingException( Exception ex, HttpServletRequest request ) {
    ...
    public ResponseEntity< ? > handleGeneralException( Throwable ex, HttpServletRequest request ) {
        ...
        return new ResponseEntity<>( error.representation( ), new HttpHeaders( ), HttpStatus.valueOf( ErrorCode.SYSTEM_ERROR.getStatus( ) ) );
    }

    private ResponseEntity< ErrorRepresentation > createErrorResponse( MryException exception, String path ) {
        String traceId = tracingService.currentTraceId( );
        Error error = new Error( exception, path, traceId );
        ErrorRepresentation representation = error.representation( );
        return new ResponseEntity<>( representation, new HttpHeaders( ), HttpStatus.valueOf( representation.error( ).getStatus( ) ) );
    }
